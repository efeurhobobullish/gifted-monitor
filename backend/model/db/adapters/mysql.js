async function createMysqlAdapter(DATABASE_URL) {
  ...
  await pool.query(`ALTER TABLE users ADD COLUMN telegram_chat_id VARCHAR(32) NULL`).catch(() => {});
  ...
  return {
    ...
    async getUserByTelegramChatId(chatId) {
      ...
      return row(await pool.query('SELECT * FROM users WHERE telegram_chat_id=?', [String(chatId)]));
    },
